...
트랜스포머 블록의 첫 단계는 이 행렬에 _레이어 정규화(layer normalization)_를 적용하는 것입니다. 이는 행렬의 각 열 값을
각각 따로 정규화하는 연산입니다.`;
    breakAfter();
...
${c_blockRef('정규화된 입력 임베딩(normalized input embedding)', ln.lnResid)}이 만들어지고, 이는 셀프 어텐션 레이어로 전달될 준비가 됩니다.
`;
