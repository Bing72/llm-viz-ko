...
const walkthroughEnglish = new Map<string, string>([
    ...
    ["시퀀스 안의 각 숫자는 먼저 48개 원소를 가진 벡터로 바뀝니다(이 크기는 이 모델에서 선택한 값입니다). 이것을 _임베딩_이라고 합니다.", "Each number in the sequence first gets turned into a 48 element vector (a size chosen for this particular model). This is called an _embedding_."],
    ["그다음 임베딩은 모델 아래쪽으로 전달되며, 트랜스포머라고 부르는 여러 층을 차례로 통과합니다.", "The embedding is then passed through the model, going through a series of layers, called transformers, before reaching the bottom."],
    ...
    ["앞에서 간단한 조회 테이블을 이용해 토큰이 정수 시퀀스로 매핑되는 과정을 봤습니다.", "We saw previously how the tokens are mapped to a sequence of integers using a simple lookup table."],
